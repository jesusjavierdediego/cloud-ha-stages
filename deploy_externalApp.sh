...
# 1-Replace values in descriptors
cat descriptors/bureau/templates/${APP}.json | sed 's/XXX_PROFILE_XXX/'${ENV}'/g' | sed 's/XXX_VERSION_XXX/'${VERSION}'/g' | sed 's/XXX_INSTANCES_XXX/'${INSTANCES}'/g' > descriptors/bureau/${APP}-deploy.json


# 2-deploy app
# dcos marathon app add descriptors/bureau/${APP}-deploy.json
...
# az network nsg rule create --resource-group $(prop 'resourceGroup') --nsg-name $nsgName --name $(prop 'demoapp1.name')-rule --access Allow --protocol Tcp --direction Inbound --priority $(prop 'demoapp1.rulenr') --source-address-prefix Internet  --destination-port-range $(prop 'demoapp1.serviceport')
 # I want to set up a plannning meeting about the cloud stuff